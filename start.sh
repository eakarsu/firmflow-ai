...
case "$mode" in
  ...
  migrate) npm --prefix "$project_dir" run migrate; exit ;;
  start) ;;
...
export BOOTSTRAP_ADMIN_EMAIL="${ADMIN_EMAIL:?ADMIN_EMAIL is required}"
export BOOTSTRAP_ADMIN_PASSWORD="${ADMIN_PASSWORD:?ADMIN_PASSWORD is required}"
export BOOTSTRAP_ADMIN_NAME="${PROVISION_ADMIN_NAME:-Runtime Administrator}"
"$project_dir/node_modules/.bin/prisma" generate
npm --prefix "$project_dir" run migrate
